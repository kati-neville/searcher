...
export default function ResultsList({
  ...
  if (!results || results.length === 0) return <div className="text-gray-500">No findings</div>
  ...
  return (
    <>
      ...
      <div className="space-y-4">
              ...
              {r.matches.map((m, j) => {
                return (
                  <div key={j} className="mb-2">
                    ...
                    <pre className="bg-gray-100 p-2 overflow-x-auto text-sm">{m.match}</pre>
                  </div>
                )
              })}
            </div>
        ...
        <div>
          <button onClick={handleCreateIssue} className="px-3 py-1 bg-red-600 text-white rounded">
          ...
          {response?.state === 'success' ? (
            <div className="bg-green-200 rounded-md p-2 mt-2">
              <p className="text-xs">
                ...
                <a href={response.message} target="_blank" className="underline">
                  link
                </a>
              </p>
            </div>
...
function AddToken({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
